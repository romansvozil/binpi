...
- Fix autocompletion (since we're using for annotation `binpi.LEInt()` the IDE's cannot guess correctly the expected type)
- Performance improvements (batch struct.un/pack should help)
- Might be also possible to instead of using custom types like `binpi.LEInt()` just make a sub-type of `int`